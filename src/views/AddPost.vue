			...
			addPost () {
				if (this.post.title != '' && this.post.content != '') {
					...
					for (let i=0; i < groupA.length; i++) {
						if (this.post.user.charAt(0) == groupA[i]) {
							this.post.color = 'blue'
						} else if (this.post.user.charAt(0) == groupB[i]) {
							this.post.color = 'purple'
						} else if (this.post.user.charAt(0) == groupC[i]) {
							this.post.color = 'pink'
						} else if (this.post.user.charAt(0) == groupD[i]) {
							this.post.color = 'orange'
						} else if (this.post.user.charAt(0) == groupE[i]) {
							this.post.color = 'yellow'
						}
					}
					...
					this.formStateSuccess = false

					// Set error if title is missing
					if (this.post.title == '') {
						this.formFieldTitleError = true
					} else {
						this.formFieldTitleError = false
					}

					// Set error if content is missing
					if (this.post.content == '') {
						this.formFieldContent = true
					} else {
						this.formFieldContent = false
					}

					// Set form error state to true
					this.formStateError = true
				}
			}
		},
